<?php
// This file is part of Moodle - https://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <https://www.gnu.org/licenses/>.

/**
 * Strings for component 'group', language 'hu', version '3.10'.
 *
 * @package     group
 * @category    string
 * @copyright   1999 Martin Dougiamas and contributors
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['addedby'] = 'Hozzáadta {$a}';
$string['addgroup'] = 'Felhasználó hozzáadása a csoporthoz';
$string['addgroupstogrouping'] = 'Csoport hozzáadása a csoportosításhoz';
$string['addgroupstogroupings'] = 'Csoportok hozzáadása/törlése';
$string['adduserstogroup'] = 'Felhasználók hozzáadása/törlése';
$string['allocateby'] = 'Tagok hozzárendelése';
$string['anygrouping'] = '[Bármely csoportosítás]';
$string['autocreategroups'] = 'Csoportok automatikus létrehozása';
$string['backtogroupings'] = 'Vissza a csoportosításokhoz';
$string['backtogroups'] = 'Vissza a csoportokhoz';
$string['badnamingscheme'] = 'Pontosan csak egy \'@\' vagy \'#\' karaktert tartalmazhat';
$string['byfirstname'] = 'Betűrendben keresztnév, majd vezetéknév szerint';
$string['byidnumber'] = 'Betűrendben azonosítószám szerint';
$string['bylastname'] = 'Betűrendben vezetéknév, majd  keresztnév szerint';
$string['createautomaticgrouping'] = 'Automatikus csoportosítás létrehozása';
$string['creategroup'] = 'Csoport létrehozása';
$string['creategrouping'] = 'Csoportosítás létrehozása';
$string['creategroupinselectedgrouping'] = 'Csoport létrehozása csoportosításon belül';
$string['createingrouping'] = 'Automatikusan létrehozott csoportok csoportosítása';
$string['createorphangroup'] = 'Szólócsoport létrehozása';
$string['csvdelimiter'] = 'CSV-elválasztó';
$string['databaseupgradegroups'] = 'A jelenlegi csoportverzió most: {$a}';
$string['defaultgrouping'] = 'Alapértelmezett csoportosítás';
$string['defaultgroupingname'] = 'Csoportosítás';
$string['defaultgroupname'] = 'Csoport';
$string['deleteallgroupings'] = 'Az összes csoportosítás törlése';
$string['deleteallgroups'] = 'Az összes csoport törlése';
$string['deletegroupconfirm'] = 'Biztosan törli a(z) {$a} csoportot?';
$string['deletegrouping'] = 'Csoportosítás törlése';
$string['deletegroupingconfirm'] = 'Biztosan törli a(z) {$a} csoportosítást? (A benne lévő csoportok nem törlődnek.)';
$string['deletegroupsconfirm'] = 'Biztosan törli az alábbi csoportokat?';
$string['deleteselectedgroup'] = 'Kiválasztott csoport törlése';
$string['editgroupingsettings'] = 'Csoportosítás beállításainak szerkesztése';
$string['editgroupsettings'] = 'Csoport beállításainak szerkesztése';
$string['editusersgroupsa'] = '"{$a}" csoportjainak szerkesztése';
$string['enablemessaging'] = 'Csoportos üzenetküldés';
$string['enablemessaging_help'] = 'Bekapcsolása esetén a csoporttagok a csoporttársaiknak üzenetet küldhetnek az üzenetküldő fiók segítségével.';
$string['encoding'] = 'Kódolás';
$string['enrolmentkey'] = 'Beiratkozási kulcs';
$string['enrolmentkey_help'] = 'Beiratkozási kulccsal a kurzus elérése a kulcsot ismerőkre korlátozható. Ha van megadva beiratkozási kulcs, akkor azzal a felhasználó nemcsak beléphet a kurzusba, hanem automatikusan a csoport tagjává válik.

Megjegyzés: az önbeiratkozás beállításainál be kell kapcsolni a csoportos beiratkozást és a kurzushoz meg kell adni egy beiratkozási kulcsot.';
$string['enrolmentkeyalreadyinuse'] = 'Ezt a beiratkozási kulcsot már egy másik csoport használja.';
$string['erroraddremoveuser'] = 'Hiba a(z) {$a} felhasználó csoporthoz való hozzáadása/csoportból való törlése közben';
$string['erroreditgroup'] = 'Hiba a(z) {$a} csoport létrehozása/frissítése közben';
$string['erroreditgrouping'] = 'Hiba a(z) {$a} csoportosítás létrehozása/frissítése közben';
$string['errorinvalidgroup'] = 'Hiba, érvénytelen {$a} csoport';
$string['errorremovenotpermitted'] = 'Az automatikusan hozzáadott  {$a} csoporttag törléséhez nincs engedélye';
$string['errorselectone'] = 'Ennek kiválasztása előtt válasszon ki egy csoportot.';
$string['errorselectsome'] = 'Ennek kiválasztása előtt válasszon ki egy vagy több csoportot.';
$string['evenallocation'] = 'Megjegyzés: A csoport-hozzárendelés egyenletes volta érdekében a tagok csoportonkénti száma eltér az Ön által megadottól.';
$string['eventgroupcreated'] = 'Csoport létrehozva';
$string['eventgroupdeleted'] = 'Csoport törölve';
$string['eventgroupingcreated'] = 'Csoportosítás létrehozva';
$string['eventgroupingdeleted'] = 'Csoportosítás törölve';
$string['eventgroupinggroupassigned'] = 'Csoportosításhoz hozzárendelt csoport';
$string['eventgroupinggroupunassigned'] = 'Csoportosításból kivett csoport';
$string['eventgroupingupdated'] = 'Csoportosítás frissítve';
$string['eventgroupmemberadded'] = 'Csoporttag hozzáadva';
$string['eventgroupmemberremoved'] = 'Csoporttag törölve';
$string['eventgroupupdated'] = 'Csoport frissítve';
$string['existingmembers'] = 'Létező tagok: {$a}';
$string['filtergroups'] = 'Csoportok szűrésének szempontja:';
$string['group'] = 'Csoport';
$string['groupaddedsuccesfully'] = 'A(z) {$a} csoport hozzáadása sikerült';
$string['groupaddedtogroupingsuccesfully'] = '{$a->groupname} csoport {$a->groupingname} csoportosításhoz hozzáadva';
$string['groupby'] = 'Automatikusan létrehozás alapja';
$string['groupdescription'] = 'Csoportleírás';
$string['groupinfo'] = 'Információk a kiválasztott csoportról';
$string['groupinfomembers'] = 'Információk a kiválasztott tagokról';
$string['groupinfopeople'] = 'Információk a kiválasztott személyekről';
$string['grouping'] = 'Csoportosítás';
$string['grouping_help'] = 'A csoportosítás egy adott kurzuson belüli csoportok összessége. Csoportosítás kiválasztása esetén az azon belül csoportokba osztott tanulók együtt tevékenykedhetnek.';
$string['groupingaddedsuccesfully'] = '"{$a}" csoportosítás hozzáadva';
$string['groupingdescription'] = 'Csoportosítás leírása';
$string['groupingname'] = 'Csoportosítás neve';
$string['groupingnameexists'] = 'A(z) \'{$a}\' nevű csoport már szerepel ebben a kurzusban, válasszon másikat.';
$string['groupings'] = 'Csoportosítások';
$string['groupingsection'] = 'Csoportosítás elérése';
$string['groupingsection_help'] = 'A csoportosítás egy kurzuson belül a csoportok összessége. Ha itt csoportosítást választ ki, csak az azon belüli valamelyik csoporthoz tartozó tanulók érhetik el az adott szakaszt.';
$string['groupingsonly'] = 'Csak csoportosítások';
$string['groupmember'] = 'Csoporttag';
$string['groupmemberdesc'] = 'Csoporttag szokásos szerepe.';
$string['groupmembers'] = 'Csoporttagok';
$string['groupmemberssee'] = 'Csoporttagok megtekintése';
$string['groupmembersselected'] = 'A kiválasztott csoport tagjai';
$string['groupmode'] = 'Csoportos részvétel';
$string['groupmode_help'] = 'A kurzusban való csoportos részvétel három szinten történhet:
* Nincs csoport
* Külön csoportok - az egyes csoportok csak a saját csoportjukat látják, a többi csoport számukra láthatatlan
* Látható csoportok - az egyes csoportok saját csoportjukon belül tevékenykednek, de láthatják a többi csoportot is

A kurzusszinten kialakított csoportos részvétel az adott kurzuson belül meghatározott összes tevékenység alapvető részvételi módja. A csoportokat támogató egyes tevékenységek meghatározhatják saját csoportosítási módjaikat. Ha a kurzus beállítása kurzusszinten előírt csoportos részvétel, akkor a rendszer figyelmen kívül hagyja az egyes tevékenységek beállítását.';
$string['groupmodeforce'] = 'Csoportos részvétel előírása';
$string['groupmodeforce_help'] = 'Csoportos részvétel előírása esetén ';
$string['groupmy'] = 'Csoportom';
$string['groupname'] = 'Csoport neve';
$string['groupnameexists'] = 'A(z) \'{$a}\' nevű csoport már szerepel ebben a kurzusban, válasszon másikat.';
$string['groupnotamember'] = 'Ön nem tagja annak a csoportnak';
$string['groups'] = 'Csoportok';
$string['groupscount'] = 'Csoport ({$a})';
$string['groupsettingsheader'] = 'Csoportok';
$string['groupsgroupings'] = 'Csoportok és csoportosítások';
$string['groupsinselectedgrouping'] = 'Csoportok itt:';
$string['groupsnone'] = 'Nincs csoport';
$string['groupsonly'] = 'Csak csoportok';
$string['groupspreview'] = 'Csoportok előnézete';
$string['groupsseparate'] = 'Külön csoportok';
$string['groupsvisible'] = 'Látható csoportok';
$string['grouptemplate'] = '@ csoport';
$string['hidepicture'] = 'Kép elrejtése';
$string['importgroups'] = 'Csoportok importálása';
$string['importgroups_help'] = 'Csoportok importálása történhet szöveges állományból. Ennek formája a következő:

* Minden sorban egy rekord szerepel.
* Minden rekord vesszővel elválasztott adatokat tartalmaz
* Az első rekord az állomány többi részének formáját meghatározó mezőnevek felsorolását tartalmazza
* Az előírt mezőnév a csoport neve
* Választható mezőnevek: groupidnumber, description, enrolmentkey, groupingname';
$string['includeonlyactiveenrol'] = 'Csak aktív beiratkozások szerepeljenek';
$string['includeonlyactiveenrol_help'] = 'Bekapcsolása esetén a felfüggesztett felhasználók kimaradnak a csoportokból.';
$string['javascriptrequired'] = 'Ehhez az oldalhoz be kell kapcsolni a javascriptet.';
$string['memberofgroup'] = 'A(z) {$a} csoport tagja';
$string['members'] = 'Tagok csoportonként';
$string['membersofselectedgroup'] = 'Tagok itt:';
$string['mygroups'] = 'Csoportjaim';
$string['namingscheme'] = 'Nevezési séma';
$string['namingscheme_help'] = 'A @ szimbólummal betűket tartalmazó csoportneveket hozhat létre. Pl. a @ csoporttal létrejön az A csoport, B csoport, C csoport ... A # szimbólummal számokat tartalmazó csoportneveket hozhat létre. Pl. a # csoporttal létrejön az 1 csoport, 2 csoport, 3 csoport ...';
$string['newgrouping'] = 'Új csoportosítás';
$string['newpicture'] = 'Új kép';
$string['newpicture_help'] = 'Válasszon ki egy JPG vagy PNG formátumú képet. A kép négyzet alakú és 100x100 képernyőpont méretű lesz.';
$string['noallocation'] = 'Nincs hozzárendelés';
$string['nogroup'] = 'Nincs csoport';
$string['nogrouping'] = 'Nincs csoportosítás';
$string['nogroups'] = 'Ebben a kurzusban még nincsenek beállítva csoportok';
$string['nogroupsassigned'] = 'Nincsenek hozzárendelve csoportok';
$string['nopermissionforcreation'] = 'Nem hozhatja létre a(z) \'{$a}\' csoport, mivel nincs meg hozzá a szükséges jogosultsága.';
$string['nosmallgroups'] = 'Utolsó kis csoport megakadályozása';
$string['notingroup'] = 'Csoportbeli felhasználók kihagyása';
$string['notingrouping'] = 'Nem szerepel csoportosításban';
$string['notingrouplist'] = 'Nem egy csoportban';
$string['nousersinrole'] = 'A kiválasztott szerepben nincsenek megfelelő felhasználók';
$string['number'] = 'Csoport/tag arány';
$string['numgroups'] = 'Csoportok száma';
$string['nummembers'] = 'Tagok csoportonként';
$string['othergroups'] = 'Egyéb csoportok';
$string['overview'] = 'Áttekintés';
$string['potentialmembers'] = 'Potenciális tagok: {$a}';
$string['potentialmembs'] = 'Lehetséges tagok';
$string['printerfriendly'] = 'Nyomtatóbarát megjelenítés';
$string['privacy:metadata:core_message'] = 'A csoportbeszélgetések';
$string['privacy:metadata:groups'] = 'Csoporttagság rekordja.';
$string['privacy:metadata:groups:groupid'] = 'A csoport azonosítója.';
$string['privacy:metadata:groups:timeadded'] = 'A felhasználó csoportba való felvételének időbélyegzője.';
$string['privacy:metadata:groups:userid'] = 'A csoportba tartozó felhasználó azonosítója.';
$string['random'] = 'Véletlenszerűen';
$string['removefromgroup'] = 'Felhasználó törlése a(z) {$a} csoportból';
$string['removefromgroupconfirm'] = 'Biztosan törli "{$a->user}" felhasználót a(z) "{$a->group}" csoportból?';
$string['removegroupfromselectedgrouping'] = 'Csoport törlése a csoportosításból';
$string['removegroupingsmembers'] = 'Minden csoport törlése a csoportosításokból';
$string['removegroupsmembers'] = 'Minden csoporttag törlése';
$string['removeselectedusers'] = 'Kiválasztott felhasználók törlése';
$string['selectfromgroup'] = 'Válasszon ki csoporttagokat.';
$string['selectfromgrouping'] = 'Válasszon ki tagokat a csoportosításból.';
$string['selectfromrole'] = 'Tagok kiválasztása a szerep alapján';
$string['showgroupsingrouping'] = 'Csoportosításon belüli csoportok megjelenítése';
$string['showmembersforgroup'] = 'A csoport tagjainak megjelenítése';
$string['toomanygroups'] = 'Ennyi csoport közzétételéhez kevés a felhasználó - a kiválasztott szerephez csak {$a} felhasználó van hozzárendelve';
$string['usercount'] = 'Felhasználók száma';
$string['usercounttotal'] = 'Felhasználók száma ({$a})';
$string['usergroupmembership'] = 'Felhasználó tagságának kiválasztása';
